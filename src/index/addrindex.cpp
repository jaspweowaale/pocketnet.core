    ...
    std::map<std::string, int>& postReputations)
{
    ...
    Item postItm;
    if (!g_pocketdb->SelectOne(reindexer::Query("Posts").Where("otxid", CondEq, posttxid).Where("last", CondEq, true), postItm).ok()) return false;
...
int64_t AddrIndex::GetUserRegistrationDate(std::string _address)
{
    ...
    reindexer::Error err = g_pocketdb->SelectOne(reindexer::Query("Users").Where("address", CondEq, _address).Where("last", CondEq, true), userItm);
...
bool AddrIndex::GetRecommendedPostsBySubscriptions(std::string _address, int count, std::set<string>& recommendedPosts)
{
    ...
    for (auto itsub : subscriptions) {
        ...
        reindexer::Error err = g_pocketdb->SelectOne(
            reindexer::Query("Posts").Where("address", CondEq, itsub).Where("last", CondEq, true).Sort("time", true),
            queryItm);
        if (err.ok()) {
...
bool AddrIndex::GetRecommendedPostsByScores(std::string _address, int count, std::set<string>& recommendedPosts)
{
    ...
    if (err.ok() && queryRes1.Count() > 0) {
        ...
        if (err.ok() && queryRes2.Count() > 0) {
            ...
            reindexer::AggregationResult aggRes;
            err = g_pocketdb->SelectAggr(
                reindexer::Query("Scores").Where("address", CondSet, fellowLikers).Where("value", CondSet, {4, 5}).Aggregate(AggFacet, {"posttxid"}),
                "posttxid",
                aggRes);
...
bool AddrIndex::GetTXRIData(CTransactionRef& tx, std::string& data)
{
    ...
    // Prepare data for rend to another node
    {
        ...
        if (!mempool && ri_table == "Users") {
            itm["last"] = true;
            itm["reputation"] = 0;
        }

        if (!mempool && ri_table == "Posts") {
            itm["last"] = true;
            itm["scoreSum"] = 0;
            itm["scoreCnt"] = 0;
            itm["reputation"] = 0;
