...
static UniValue getlastblocks(const JSONRPCRequest& request) {
    ...
    if (verbose) {
        ...
        if (g_pocketdb->SelectAggr(reindexer::Query("User").Where("block", CondGt, last_height - count).Where("block", CondLe, last_height).Where("last", CondEq, true).Aggregate("block", AggFacet), "block", aggRes).ok()) {
        ...
        if (g_pocketdb->SelectAggr(reindexer::Query("Post").Where("block", CondGt, last_height - count).Where("block", CondLe, last_height).Where("last", CondEq, true).Aggregate("block", AggFacet), "block", aggRes).ok()) {
            for (const auto& f : aggRes.facets) {
