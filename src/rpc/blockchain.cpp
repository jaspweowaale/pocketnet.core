...
static UniValue getlastblocks(const JSONRPCRequest& request) {
    ...
    if (verbose) {
        ...
        if (g_pocketdb->SelectAggr(reindexer::Query("UsersView").Where("block", CondGt, last_height - count).Where("block", CondLe, last_height).Aggregate(AggFacet, {"block"}), "block", aggRes).ok()) {
        ...
        if (g_pocketdb->SelectAggr(reindexer::Query("Posts").Where("block", CondGt, last_height - count).Where("block", CondLe, last_height).Aggregate(AggFacet, {"block"}), "block", aggRes).ok()) {
            for (const auto& f : aggRes.facets) {
        ...
        if (g_pocketdb->SelectAggr(reindexer::Query("Comment").Where("block", CondGt, last_height - count).Where("block", CondLe, last_height).Aggregate(AggFacet, {"block"}), "block", aggRes).ok()) {
            for (const auto& f : aggRes.facets) {
